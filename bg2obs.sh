...
for book_index in "${!bookarray[@]}"; do
    ...
    for ((chapter=1; chapter <= maxchapter; chapter++)); do
        ...
        breadcrumbs_top="$prev_link $([ -n "$prev_link" ] && echo "| ")$book_link$([ -n "$next_link" ] && echo " | ")$next_link\n\n---\n\n"
        ...
        echo -e "$breadcrumbs_top$(cat "$folder_name/$filename")" > "$folder_name/$filename"
        echo -e "$breadcrumbs_bottom" >> "$folder_name/$filename"
    done
